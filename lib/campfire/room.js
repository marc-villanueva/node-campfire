...
Room.prototype.listen = function(callback) {
  ...
  var campfire = this.campfire,
      options  = {
        ...
        headers : {
          "Host"          : "streaming.campfirenow.com",
          "Authorization" : campfire.httpRequest.authorization
        }
      };

  (function listen() {
    campfire.httpRequest.http.request(options, function(response) {
...
Room.prototype.messages = function(callback, options) {
  options = options || {};
  ...
  this.get("/recent?" + params.join('&'), function(error, response) {
    if (response) {
